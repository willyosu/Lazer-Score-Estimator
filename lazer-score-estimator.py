...
class Score:
    ...
    def __init__(self, maxCombo, hitObjects, accuracy = 100.0, combo = False, modString = False):
    ...
    def calculateClassic(self):
        self.classicScore = round(36 * pow((self.standardizedScore / Score.maxScore) * (self.hitObjects + 1), 2))

class CommandLine:
    def __init__(self):
        parser = argparse.ArgumentParser(description = "Description for my parser")
        parser.add_argument("-M", "--MaxCombo", help = "(int) maximum combo of a beatmap.", required = False)
        parser.add_argument("-O", "--HitObjects", help = "(int) number of hit objects in a beatmap.", required = False)
        ...
        if(argument.MaxCombo is not None):
            play = Score(int(argument.MaxCombo), int(argument.HitObjects), float(argument.accuracy), int(argument.combo), argument.mods)
    ...
    def runApplication(self):
        ...
        hitObjects = int(input("Hit Objects: "))
        accuracy = float(input("Accuracy: "))
        combo = int(input("Combo: "))
        modList = input("Mods: ")
        play = Score(maxCombo, hitObjects, accuracy, combo, modList)
